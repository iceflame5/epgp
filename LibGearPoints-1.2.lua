...
local CUSTOM_ITEM_DATA = {
 ...
 -- T14 heroic
  ...
  -- Shadowlands weapon tokens -- ie https://shadowlands.wowhead.com/item=183891/venerated-anima-spherule
  [183890] = { 4, 0, "INVTYPE_WEAPONMAINHAND", false },
  [183891] = { 4, 0, "INVTYPE_WEAPONMAINHAND", false },
  [183892] = { 4, 0, "INVTYPE_WEAPONMAINHAND", false },
  [183893] = { 4, 0, "INVTYPE_WEAPONMAINHAND", false },
  [183896] = { 4, 0, "INVTYPE_WEAPONMAINHAND", false },
  [183897] = { 4, 0, "INVTYPE_WEAPONMAINHAND", false },
  [183898] = { 4, 0, "INVTYPE_WEAPONMAINHAND", false },
  [183899] = { 4, 0, "INVTYPE_WEAPONMAINHAND", false },

  -- Shadowlands offhand tokens -- ie https://shadowlands.wowhead.com/item=183888/apogee-anima-bead
  [183888] = { 4, 0, "INVTYPE_WEAPONOFFHAND", false },
  [183889] = { 4, 0, "INVTYPE_WEAPONOFFHAND", false },
  [183894] = { 4, 0, "INVTYPE_WEAPONOFFHAND", false },
  [183895] = { 4, 0, "INVTYPE_WEAPONOFFHAND", false },
}
...
local ITEM_BONUS_SOCKET_GP = 300
local ITEM_BONUS_GP = {
  [40]  = 50,  -- avoidance
  [41]  = 50,  -- leech
  [42]  = 50,  -- speed
...
function lib:GetValue(item)
  ...
  local x1 , itemLink, rarity, level, _, itemClass, itemSubClass, _, equipLoc, _, _, classID, subclassID, bindType, expacID, setID  = GetItemInfo(item)

  -- itemID, itemType, itemSubType, itemEquipLoc, icon, itemClassID, itemSubClassID = GetItemInfoInstant(.)
  ...
  -- for these).
  if level < 180 and not CUSTOM_ITEM_DATA[itemID] then -- TODO: what level to check for now?
  ...
  if CUSTOM_ITEM_DATA[itemID] then
    rarity, newlevel, equipLoc, useItemBonuses = unpack(CUSTOM_ITEM_DATA[itemID])
    if (newlevel > 0) then
      level = newlevel
    end
  ...
  --[[
  Debug("Item %s, %d, %s, %d, %d, %d", x1, level, equipLoc, rarity, classID, subclassID)
  if equipLoc == "" and classID == 5 and subclassID == 2 then
    printable = gsub(itemLink, "\124", "\124\124")
    Debug("link: %s", printable)
  end
  ]]--
  ...
  if not stats then
    ...
    for k,v in pairs(stats) do
      if string.find(k, "EMPTY_SOCKET_") then
          socketNum = socketNum + v
      end
    end
  end
  extra_gp = extra_gp + ((socketNum * ITEM_BONUS_SOCKET_GP) or 0)

  --[[
  SocketNum = function(itemLink)
      local total = 0
      local stats = GetItemStats(itemLink)
  ...
  elseif version < 90000 then
    ...
    standard_ilvl = 200 -- Castle of Nathria (normal - hc is 213)
    ilvl_denominator = 24
  end
